...
final class WeatherServiceTests: XCTestCase {
    ...
    func testSearchCitiesSuccess() {
        ...
        let mockCities = [
            ...
            CitySuggestion(id: 2, name: "London", region: "Ontario", country: "Canada", lat: 42.98, lon: -81.24, url: "http://test.com")
        ]
        ...
        // When
        weatherService.searchCities(query: "London") { result in
            // Then
            switch result {
            case .success(let cities):
                ...
                XCTAssertEqual(cities[1].country, "Canada")
            case .failure(let error):
                XCTFail("Expected success but got error: \(error)")
            }
            expectation.fulfill()
    ...
    func testSearchCitiesInvalidURL() {
        // Given
        let expectation = XCTestExpectation(description: "Search cities invalid URL")
        
        // When
        weatherService.searchCities(query: "") { result in
            // Then
            switch result {
            case .success:
                XCTFail("Expected failure but got success")
